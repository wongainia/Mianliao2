...
//        mContext.startActivity(new Intent(mContext, DressUpMallActivty.class));
        ...
        MobclickAgent.onEvent(mContext, MStaticInterface.CHAT);
    }


    private void viewMarket(int type) {
        String params = new StringBuilder("type=").append(type).toString();
        String url = HttpUtil.getUrl(mContext, MsRequest.IMRW_RESOURCE_LIST, params);
        Intent iMarket = new Intent(mContext, AvatarMarketActivity.class);
    ...
    private void addEmptyAreaBg() {
        ...
        if (mCurrentWidth < mTotleWidth) {
            ...
            LayoutParams params = new LayoutParams(mTotleWidth - mCurrentWidth,
                    LayoutParams.MATCH_PARENT);
            tv.setBackgroundResource(R.drawable.pic_bg_emo);
