...
public abstract class CfRecord implements Parcelable {
    ...
    public String getTimeAndSchool(CfPost post, boolean showCreateTime) {
        StringBuilder sb = new StringBuilder(Utils.getPostShowTimeString(
                showCreateTime ? createdOn : replyTime));
        boolean sameSchool = post.sameSchool;
        if (!sameSchool) {
            sb.append(" · ").append(MianLiaoApp.getAppContext().getString(
                    R.string.cf_other_school_flag));
        }
        return sb.toString();
    }
    
    public String getTimeAndSchool(CfPost post) {
        return getTimeAndSchool(post, false);
    }
    ...
    public static class AtUser implements Parcelable {
        ...
        public static final JsonUtil.ITransformer<AtUser> TRANSFORMER =
                new JsonUtil.ITransformer<AtUser>() {
